<script lang="ts">
  ...
  function getDateAndTimePart(when: Date): string {
    if (when.getFullYear() === new Date().getFullYear()) {
      return when.toLocaleString('default', {
        day: 'numeric',
        month: 'short',
        hour: 'numeric',
        minute: 'numeric'
...
<div class="{isMessageFromUser ? 'place-self-end' : 'place-self-start'} space-y-1">
  <div
    ...
    on:focus={setfocus}
    on:blur={setblur}
    class="p-3 rounded-xl {isMessageFromUser
      ? 'bg-indigo-200 rounded-tr-none'
      : 'bg-teal-200 rounded-tl-none'}"
  >
    <p>{what}</p>
  </div>
  {#if $isFocused}
    <div transition:slide>
      <p
        transition:fade
        class={isMessageFromUser ? 'text-indigo-400 text-right' : 'text-teal-400 text-left'}
      >
        {whenString}
      </p>
    </div>
